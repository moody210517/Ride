...
class Revenue extends MY_Controller {
	...
	/**
	*
	* Initiate to display the site revenue page
	* 	
	* @return HTTP request to show the site revenue page
	*	
	**/	
    public function index() {
        if ($this->checkLogin('A') == '') {
            ...
            redirect(ADMIN_ENC_URL.'/revenue/display_site_revenue');
        }
    }

	/**
	* 
	* Display the revenue list for site and driver
	*
	...
	* @return HTML to show the total revenue and individual earnings for site and driver
	*
	**/	
        ...
        if ($this->checkLogin('A') == '') {
            ...
            if ($driversCount > 500) {
                ...
                if(!isset($_GET['range']) && $dateFrom != '' && $dateTo != ''){
					...
					$searchbaseUrl = ADMIN_ENC_URL.'/revenue/display_site_revenue?' . $manRange;
				} else {
					$searchbaseUrl = ADMIN_ENC_URL.'/revenue/display_site_revenue?range=' . $req;
				}
            ...
            if (count($dates) == 2) {
                ...
                if ($billingsList->num_rows() == 0) {
                    ...
                    $mfrom = date("m/d/Y", strtotime("+1 day", MongoEPOCH($billingsList->row()->bill_period_to)));
                }
            }
            ...
            if ($billingsList->num_rows() == 0) {
                ...
                $last_bill = date("m/d/Y", strtotime("+1 day", MongoEPOCH($billingsList->row()->bill_period_to)));
            }
            ...
            $rideSummary = $this->revenue_model->get_ride_summary($fdate, $tdate,'',$locationId);
            ...
            $this->load->view(ADMIN_ENC_URL.'/revenue/display_site_revenue', $this->data);
        }
    }

	/**
	* 
	* Display individual ride revenue list for driver depends upon the date wise filter
	*
    ...
    public function driver_trip_summary($driver_id) {
        if ($this->checkLogin('A') == '') {
            ...
            if ($driverDetails->num_rows() > 0) {
                ...
                $this->load->view(ADMIN_ENC_URL.'/revenue/display_driver_trip_summary', $this->data);
            }
        }
    }
