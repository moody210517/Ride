...
class Mileage extends MY_Controller {
	...
	**/
    public function index() {
        if ($this->checkLogin('A') == '') {
            ...
            redirect(COMPANY_NAME.'/mileage/display_mileage_list');
        }
    }
	...
	**/	
    public function display_mileage_dashboard() {
		...
		$this->load->view(COMPANY_NAME.'/mileage/mileage_dashboard', $this->data);
	}
    ...
    public function display_mileage_list() {
		...
		if($this->data['driversList']->num_rows() > 0 ){
			foreach($this->data['driversList']->result() as $row){
				$categoryVal=$this->user_model->get_selected_fields(CATEGORY,array('_id'=>$row->category),array('name','email','_id'));
				$driver_array[]=$row->_id;
				$cat_name = "";
				if($categoryVal->num_rows()>0) $cat_name = $categoryVal->row()->name;
		...
		$mileage_data=get_mileage_list($driver_array,$start_date,$end_date,$company_id);
		$total_duration=0;
		$total_distance=0;
		$tot_free_distance=0;
		$tot_free_duration=0;
		...
		$cabCats = $this->mileage_model->get_selected_fields(CATEGORY, array(), array('_id', 'name'))->result();
		$cabsTypeArr = array();
		...
		$this->data['total_mileage_info'] = get_total_mileage_list($driver_array,$start_date,$end_date,$company_id);
		...
		$this->load->view(COMPANY_NAME.'/mileage/display_mileage_list', $this->data);
    }
	...
	**/
    public function view_driver_mileage() {
		...
		$this->load->view(COMPANY_NAME.'/mileage/view_driver_mileage', $this->data);
    }
		...
		for($i=0; $i<$no_of_sheets; $i++){
			...
			foreach($mileage_data['result'] as $key=>$val){
				if($key >= $current_limit && $key < $next_limit){
					...
					$from_time=date('Y-m-d h:i:s A',MongoEPOCH($mileage_data['result'][$key]['mileage_data']['start_time']));

					$this->excel->getActiveSheet()->setCellValue($contentLetter.$row, $from_time);
					$contentLetter++;

					$end_time=date('Y-m-d h:i:s A',MongoEPOCH($mileage_data['result'][$key]['mileage_data']['end_time']));
...
/* Location: ./application/controllers/company/mileage.php */