...
<html xmlns="http://www.w3.org/1999/xhtml">
			...
			<div class="shipping_address">
				<div class="main">		
					<div class="app-content-box ">
						...
						<h1><?php if ($this->lang->line('wallet_choose_payment_method') != '') echo stripslashes($this->lang->line('wallet_choose_payment_method')); else echo 'Choose Your Payment Method'; ?><a class="close_btn" href="<?php echo base_url(); ?>v8/webview/wallet/cancel?mobileId=<?php echo $trans_details->row()->transaction_id;?>"></a></h1>
						
						<ul>
							<li>
								<?php if($paypal_settings['status'] == 'Enable'){ ?>
								<span class="paypalBtn">
								<label><?php if ($this->lang->line('wallet_paypal') != '') echo stripslashes($this->lang->line('wallet_paypal')); else echo 'PayPal'; ?></label>
								...
								<label> <?php if ($this->lang->line('wallet_credit_card') != '') echo stripslashes($this->lang->line('wallet_credit_card')); else echo 'Credit Card'; ?> </label>
					...
					<div class="app-content-box payment_container" style="display:block;" id="paypal_container">
						<form name="PaymentForm" id="PaymentPalForm" method="post" enctype="multipart/form-data" action="<?php echo base_url(); ?>v8/api/wallet_payment/paypal_wal_payment_process">
	...
	<!------------------------------------------------------------------------------------------------------------------------->
					...
					<div class="app-content-box payment_container" style="display:none;" id="credit_card_container">
						<form name="PaymentCard" id="PaymentCard" method="post" enctype="multipart/form-data" action="<?php echo base_url(); ?>v8/api/wallet_payment/user_wal_PaymentCard" >
							<ul>
							...
							<input type="hidden" value="<?php echo $trans_details->row()->total_amount;?>" name="total_amount" />
								...
								<li class="last">
									<input type="submit" id="submit_btn" class="input-submit-btn" value="<?php if ($this->lang->line('wallet_proceed_pay') != '') echo stripslashes($this->lang->line('wallet_proceed_pay')); else echo 'Proceed to Pay'; ?> <?php echo $this->config->item('currency_symbol').' '.number_format($trans_details->row()->total_amount,2); ?>" onClick="return validatecard();"></input>
								</li>
