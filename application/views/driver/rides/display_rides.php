...
$startdate = get_time_to_string('Y-m-d',strtotime($driver_info->row()->created));
   ...
   <div  class="profile_login_cont">
      ...
      <div class="share_detail">
         <div class="share_det_title">
            <h2><span>  <?php
							if ($this->lang->line('driver_my_rides') != '')
                                        echo stripslashes($this->lang->line('driver_my_rides'));
                                    else
                                        echo 'MY RIDES';
                    ?> </span></h2>
         </div>
         <div class="profile_ac_inner_det">
            <div class="profile_ac_details profile_ac_title myride_part">
               <p class="search_title_top">  <?php
                                    if ($this->lang->line('dash_user_search_rides_by_date') != '')
                                        echo stripslashes($this->lang->line('dash_user_search_rides_by_date'));
                                    else
                                        echo 'Search rides by date';
                                    ?>...</p>
               <span class="from">
               ...
               <button class="rd_btn rdd_btn" onclick="datechange();"><?php
                                    if ($this->lang->line('driver_filter_search') != '')
                                        echo stripslashes($this->lang->line('driver_filter_search'));
                                    else
                                        echo 'Search';
                                    ?> </button>
               <button class="rd_btn rdd_btn" onclick="resetDate('<?php echo $startdate; ?>','<?php echo $enddate; ?>');"><?php
                                    if ($this->lang->line('driver_filter_reset') != '')
                                        echo stripslashes($this->lang->line('driver_filter_reset'));
                                    else
                                        echo 'Reset';
                                    ?></button>
            </div>
            <div class="profile_ac_form myride">
               <table id="display_rides_tbl" class="table table-striped table-bordered" cellspacing="0" width="100%">
                  <thead>
                     <tr>
                        <th>  <?php
                                    if ($this->lang->line('driver_ride_id') != '')
                                        echo stripslashes($this->lang->line('driver_ride_id'));
                                    else
                                        echo 'Ride ID';
                                    ?> </th>
                        <th> <?php
                                    if ($this->lang->line('driver_ride_driver') != '')
                                        echo stripslashes($this->lang->line('driver_ride_driver'));
                                    else
                                        echo 'Driver';
                                    ?> </th>
                        <th> <?php
                                    if ($this->lang->line('driver_ride_trip_date') != '')
                                        echo stripslashes($this->lang->line('driver_ride_trip_date'));
                                    else
                                        echo 'Trip Date';
                                    ?> </th>
                        <th> <?php
                                    if ($this->lang->line('driver_ride_status') != '')
                                        echo stripslashes($this->lang->line('driver_ride_status'));
                                    else
                                        echo 'Status';
                                    ?> </th>
                        <th> <?php
                                    if ($this->lang->line('driver_ride_car') != '')
                                        echo stripslashes($this->lang->line('driver_ride_car'));
                                    else
                                        echo 'Car';
                                    ?> </th>
                        <th> <?php
                                    if ($this->lang->line('driver_ride_invoice') != '')
                                        echo stripslashes($this->lang->line('driver_ride_invoice'));
                                    else
                                        echo 'Invoice';
                                    ?> </th>
                     </tr>
                  ...
                  <tbody>
					...
					?>
                     <tr>
                        ...
                        <td><a href="driver/rides/view_ride_details/<?php echo $rides->ride_id; ?>"><?php
...
<?php $startTimeStr = strtotime($driver_info->row()->created); ?>
