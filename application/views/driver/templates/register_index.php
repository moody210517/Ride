...
<section class="ddriver_reg_banner">
	<div class="driver_banner_img">
	<!--<img src="images/driver_reg_banner.jpg" width="100%">-->
	</div>
	<div class="driver_reg_form">
		<div class="container">
			<div class="row">
				<div class="col-md-6 col-lg-6">
					<div class="driver_form_detail">
						<h1>
						<?php
				...
				<div class="col-md-6 col-lg-6">
					<div class="driver_register_form">
						...
						echo form_open('driver/profile/temp_register',$formArr);
						?>
							...
							<fieldset id="first_set">
								<div class="f_row">
									...
									<?php 
										...
										$drop_options = array('" data-catId="' => $city);
										foreach ($locationList->result() as $loc) {  
											$optkey = (string)$loc->_id.'" data-catId="'.(string)$loc->_id; 
											$drop_options[$optkey] = $loc->city;
										}
										
										$input_data = 'class="required" id="driver_location"';
										
										echo form_dropdown('driver_location',$drop_options,'',$input_data);
										
									?>
								</div>
								<div class="f_row" style="display:none;" id="catBox">
									...
									<?php 
										...
										echo form_dropdown('category',$drop_options,'',$input_data);
								...
								<div class="f_row">
								   <div class="d_email">
										...
										<?php 
											...
											echo form_input($input_data);
										?>
									  <span id="email_exist" class="error" style="color: #de5130 !important;"></span>
								   </div>
								</div>
								...
								<div class="f_row">
									...
									<?php 
										...
										$drop_options = array('' => $gender_title,
																		'male' => $male,
																		'female' => $female
																);
										...
										echo form_dropdown('gender',$drop_options,'',$input_data);
										
									?>
								</div>
								
								<div class="f_row init_box_mob">
									...
									<?php 
										...
										$input_data = 'class="required chzn-select" id="init_box_country_code"';
							...
							<fieldset id="second_set">
								...
								<div class="f_row">	
									<input type="button" id="otp_send_btn" class="otpbtns dr_otp_btn" value="Resend OTP" data-otptype="RO" onclick="sendOtp();">
								</div>
...
<section class="driver_reg_feature">
   <div class="container">
      <div class="row">
         <div class="col-md-1 col-lg-1"></div>
         <div class="col-md-5 col-lg-5">
            <div class="dirver_money">
               <div class="icon">
               </div>
               <div class="detail">
                  <h2>Make good money</h2>
                  <h6>Use your Ride to earn income</h6>
                  <p>Use your own Ride to earn cash, we cater for all types of transportation services, from comfort cars, taxis, buses, trucks etc. Your Ride will never be idle again.</p>
               </div>
            </div>
         </div>
         <div class="col-md-5 col-lg-5">
            <div class="driver_timeing">
               <div class="icon">
               </div>
               <div class="detail">
                  <h2>Register with us</h2>
                  <h6>Simple process to register with us!</h6>
                  <p>It takes a few steps to register with us. Don’t hesitate any longer. Our driver or vehicle verification process is swift and hassle free.</p>
               </div>
            </div>
         </div>
         <div class="col-md-1 col-lg-1"></div>
...
<input type="hidden" id="ajaxProgress" value="0" >
...
<script>
	$(document).ready(function(){
        ...
        $("#email").keydown(function(){ 
		 ...
		 $("#init_box_mobile_number").keydown(function(){ 
            $('#init_box_otpNumErr').html('');
        });
		...
		$('#driver_location').change(function(){ 
			...
			if(locId != '' && locId != 'undefined'){
				var progCount = Number($('#ajaxProgress').val())+1;
				$('#ajaxProgress').val(progCount);
				$.ajax({
				    type: "POST",
				    url: 'driver/profile/available_categories',
				    data: {'locId':locId},
				    dataType: 'json',
	...
	function submit_register_form(){
		...
		if($('#driver_register_form').valid()){
			...
			if(otp_verify_status == 'Yes'){ 
				...
				//$('#formErr').html('<?php  if ($this->lang->line('driver_otp_verification_process_pending') != '') echo stripslashes($this->lang->line('driver_otp_verification_process_pending')); else echo 'OTP verification process is pending'; ?>');
				setTimeout(function() { $('#formErr').hide(); }, 2500);
			}
		}
	}
</script>
