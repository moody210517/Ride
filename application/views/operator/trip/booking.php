			...
			<div class="grid_12">
				<div class="widget_wrap">
					...
					<div class="widget_content">
						<form id="book_trip" method="POST" class="form_container left_label" action="<?php echo OPERATOR_NAME; ?>/trip/confirm_trip">
									...
									<div class="form_grid_12">
										<div class="form_input">
											...
											<?php foreach ($countryList as $country) { ?>
												<option value="<?php echo $country->dial_code; ?>" <?php if($country->dial_code==$d_country_code){ echo "selected='selected'"; } ?> label="<?php echo $country->name.' ('.$country->dial_code.')'; ?>"><?php echo $country->dial_code; ?></option>
											<?php } ?>
											</select>
...
<script>
	function estimate_fare(){
		...
		if(pickup_latlang != '' && drop_latlang != ''){
			$.ajax({
			  type: "POST",
			  url: "<?php echo OPERATOR_NAME; ?>" + "/trip/ajax_estimate_fare",
...
$(document).ready(function() {
	...
	$('#user_email').on('blur',function(){
		if($(this).val() != ''){
			$.ajax({
				url: "<?php echo OPERATOR_NAME; ?>/trip/check_email_exists",
				data: {
					'email': $(this).val()
				},
				type: "POST",
	...
	$('#mobile_number').on('blur',function(){
		if($(this).val() != ''){
			$.ajax({
				url: "<?php echo OPERATOR_NAME; ?>/trip/check_phone_number_exists",
				data: {
...
<script>
	...
	function initialize() {
        ...
        google.maps.event.addListener(autocomplete, 'place_changed', function () {
			...
			$.ajax({
				url: '<?php echo OPERATOR_NAME; ?>/trip/get_category_from_location',
    ...
    function get_nearest_drivers(){
        ...
        if(cat_id != '' && pickup_lat != '' && pickup_lon != ''){
            ...
            $('#catLoader').show();
            $.ajax({
				url: '<?php echo OPERATOR_NAME; ?>/trip/get_nearest_drivers_from_ajax',
