        ...
        <div class="grid_12">
            <div class="widget_wrap">
                ...
                <div class="widget_content chenge-pass-base">
                    <form id="book_trip" method="POST" class="form_container left_label" action="<?php echo ADMIN_ENC_URL;?>/rides/book_trip">
                        <ul class="leftsec-contsec">
                            ...
                            <li id="customers_container" style="display: <?php if($customer_type == 'new') echo 'none';?>;">
                                    <div class="form_grid_12">
                                            ...
                                            <div class="form_input">
                                                    <input type="text" id="user_autosearch" name="user_autosearch" class="large required" style="width: 50% !important;  height: 35px;" autocomplete="off"/>
...
$(document).ready(function(){
	...
	$("#user_autosearch").keyup(function(e){
        
        if(e.keyCode == 8 || e.ctrlKey == true){
            $("#suggesstion-box>li.active_li").removeClass("active_li");
        }
        
        e.preventDefault();
        
        if($("#user_autosearch").val() != ''){
            if($('.active_li').html() == null){
                $('#user_autosearch').css('border','solid 1px #d8d8d8'); 
                $.ajax({
                    type: "POST",
                    url: "<?php echo ADMIN_ENC_URL;?>/rides/ajax_user_autosearch",
                    ...
                    success: function(data){
                        ...
                        if(e.keyCode == 40){ 
                            var txt = $('#suggesstion-box li:first').html();
                            if(txt != null && txt != ''){
                                $('#user_autosearch').val(txt);
                                $('#user_id').val($('#suggesstion-box li:first').attr('id'));
                                $('#suggesstion-box li:first').addClass('active_li');
                            }
                        }
                ...
                if(e.keyCode == 40 || e.keyCode == 38){
                    if(e.keyCode == 40){
                        var nxt_active_id = $('.active_li').next('li').attr('id');
                    } else if(e.keyCode == 38){  
                        var nxt_active_id = $('.active_li').prev('li').attr('id'); 
                    } 
                    if(nxt_active_id != null && nxt_active_id != 'undefined'){
                        $('#user_autosearch').val($('#'+nxt_active_id).html());
                        $('#user_id').val(nxt_active_id);
                        $('#'+nxt_active_id).addClass('active_li');
                        $('#'+active_id).removeClass('active_li');
                    } 
