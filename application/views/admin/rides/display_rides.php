...
<script>
	...
	$(function (){
		...
		$("#export_rides").click(function(event){
			...
			window.location.href = "<?php echo base_url().ADMIN_ENC_URL; ?>/rides/display_rides?" + query_strings + "&export=excel&export_type=limited";
		});
		...
		$("#export_all_rides").click(function(event){
			event.preventDefault();
			var query_strings = "<?php echo $_SERVER["QUERY_STRING"]; ?>";
			window.location.href = "<?php echo base_url().ADMIN_ENC_URL; ?>/rides/display_rides?" + query_strings + "&export=excel&export_type=all";
		});
		
	});
	function submit_ride_filter(){
		get_field_values();
		window.location.href = "<?php echo base_url().ADMIN_ENC_URL; ?>/rides/display_rides?act=<?php if(isset($_GET['act']) && $_GET['act'] != ''){ echo $_GET['act'];} ?>&from="+$filter_from+'&to='+$filter_to+'&location='+$rideLocation;
	}
</script>
<script>
...
<div id="content" class="just-booked-img">
    <div class="grid_container">
						...
						<div class=" filter_wrap">
							<div class="widget_top filter_widget">								
								...
								<div class="btn_30_light" style="width: 80%;">
										...
										echo form_open(ADMIN_ENC_URL.'/rides/display_rides', $attributes)
										?>
        ...
        echo form_open(ADMIN_ENC_URL.'/rides/change_rides_status_global', $attributes)
        ?>
        <div class="grid_12">
                    ...
                    <table class="display display_tbl" id="<?php echo $tble; ?>">
                        <thead>
                            <tr>
                                ...
                                <?php if ($allPrev == '1' || in_array('0', $rides)) { ?>
                                    <th>
                                    ...
                                    <tr>
										    ...
										    <?php if ($allPrev == '1' || in_array('2', $rides)) { ?>
                                                <td class="center">
                                                    <ul class="action_list ">
                                                        <li style="width:100%;">
                                                            <a class="p_car tipTop" href="<?php echo ADMIN_ENC_URL;?>/rides/available_drivers_list/<?php echo $row->_id; ?>" title="<?php if ($this->lang->line('admin_rides_assign_cab') != '') echo   stripslashes($this->lang->line('admin_rides_assign_cab')); else echo 'Assign Cab'; ?>">
                                                                <?php if ($this->lang->line('admin_rides_assign_cab') != '') echo stripslashes($this->lang->line('admin_rides_assign_cab')); else echo 'Assign Cab'; ?>
                                                            </a>
                                                        </li>
                                                    </ul>
                                        ...
                                        <?php if ($allPrev == '1' || in_array('0', $rides)) { ?>
                                            <td class="center">
                                                <ul class="action_list">
                                                    <li style="width:100%;">
                                                        <a class="p_edit tipTop" href="<?php echo ADMIN_ENC_URL;?>/rides/view_ride_details/<?php echo $row->_id; ?>?act=<?php echo $this->input->get('act'); ?>" title="<?php if ($this->lang->line('admin_rides_view_details') != '') echo   stripslashes($this->lang->line('admin_rides_view_details')); else echo 'View Details'; ?>">
