						...
						?>
							<div id="tab1">
								...
								<ul class="last-btn-submit">
									<li>
										<div class="form_grid_12">
											<div class="form_input">
												<a  href="<?php echo ADMIN_ENC_URL;?>/drivers/display_drivers_list" class="btn_small btn_blue" ><span><?php if ($this->lang->line('admin_drivers_back_to_driver_list') != '') echo stripslashes($this->lang->line('admin_drivers_back_to_driver_list')); else echo 'Back To Drivers List'; ?></span></a>
											</div>
										</div>
									</li>
								</ul>
							...
							<div id="tab3">
								...
								<div class="form_grid_12">
									...
									<a class="btn_small btn_blue" href="javascript:void(0);" id="docx_vrf_<?php echo $docx_key; ?>" onclick="verify_docx('<?php echo $docx_key; ?>','<?php echo $driver_details->_id;?>','driver','<?php echo $docx_status_org; ?>');" title="<?php echo $tooltip; ?>"  data-doc_status="<?php echo $docx_status_org; ?>" style="<?php echo $state_color; ?>!important;border-color:transparent!important;float:right"><?php echo $docx_status; ?></a>
									<?php } ?>
								</div>
								</li>
						<?php 
								...
								<li>
									<div class="form_grid_12">
										...
										?>
										   <?php 
											if ($allPrev == '1' || in_array('2', $driver)) {?>
											<a class="btn_small btn_blue" href="javascript:void(0);" id="docx_vrf_<?php echo $veh_docx_key; ?>" onclick="verify_docx('<?php echo $veh_docx_key; ?>','<?php echo $driver_details->_id;?>','vehicle','<?php echo $docx_status_org; ?>');" data-doc_status="<?php echo $docx_status_org; ?>" style="<?php echo $state_color; ?> style="<?php echo $state_color; ?>!important;border-color:transparent!important;float:right"><?php echo $docx_status; ?></a>
										<?php } ?>
										</div>
									</div>
								</li>
								...
								<ul class="leftsec-contsec">
									...
									<li>
										<div class="form_grid_12">
											...
											<div class="form_input">
												<p><?php if ($this->lang->line('admin_drivers_no_record_found') != '') echo stripslashes($this->lang->line('admin_drivers_no_record_found')); else echo 'No records found'; ?></p>
												<a href="<?php echo ADMIN_ENC_URL;?>/drivers/banking/<?php echo $driver_details->_id; ?>" class="expiry_box"><?php if ($this->lang->line('admin_drivers_connect_banking') != '') echo stripslashes($this->lang->line('admin_drivers_connect_banking')); else echo 'Connect Banking'; ?></a>
											</div>
										</div>
									</li>
	...
	$.get('<?php echo ADMIN_ENC_URL;?>/drivers/document_verify_status_ajax?docxId='+docxNo+'&driverId='+driverNo+'&docx_state='+docx_state_org+'&docxType='+docxType, function(res){
		if(res == 'Success'){
			if(docx_state == Verify){
				...
				$('#docx_vrf_'+docxNo).attr("data-doc_status","Verified");
				$('#docx_vrf_'+docxNo).attr('title','Click to Unverify');
			} else {
				$('#docx_vrf_'+docxNo).html(Verify);
				$('#docx_vrf_'+docxNo).css('background','#e84c3d');
				$('#docx_vrf_'+docxNo).attr("data-doc_status","Verify");
                $('#docx_vrf_'+docxNo).attr('title','Click to Verify');
			}
		} else{
