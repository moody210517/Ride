...
<div id="content">
		<div class="grid_container">
			<?php 
				...
				echo form_open(ADMIN_ENC_URL.'/cms/change_help_status_global',$attributes) 
			?>
			<div class="grid_12">
					...
					<div class="widget_content">
						...
						<tr>
							...
							if ($allPrev == '1' || in_array('2', $cms)){
								...
								if ($mode == '0'){
							?>
								<a title="<?php if ($this->lang->line('admin_click_to_unpublish') != '') echo stripslashes($this->lang->line('admin_click_to_unpublish')); else echo 'Click to unpublish'; ?>" class="tip_top" href="javascript:confirm_status('<?php echo ADMIN_ENC_URL;?>/cms/change_help_status/<?php echo $mode;?>/<?php echo $row->id;?>');"><span class="badge_style b_done"><?php echo $row->status;?></span></a>
							<?php
								}else {	
							?>
								<a title="<?php if ($this->lang->line('admin_click_to_publish') != '') echo stripslashes($this->lang->line('admin_click_to_publish')); else echo 'Click to publish'; ?>" class="tip_top" href="javascript:confirm_status('<?php echo ADMIN_ENC_URL;?>/cms/change_help_status/<?php echo $mode;?>/<?php echo $row->id;?>')"><span class="badge_style"><?php echo $row->status;?></span></a>
							<?php 
								}
							}else {
							?>
							...
							<td class="center">
							<?php if ($allPrev == '1' || in_array('2', $cms)){?>
								<span><a class="action-icons c-edit" href="<?php echo ADMIN_ENC_URL;?>/cms/edit_help/<?php echo $row->id;?>" title="<?php if ($this->lang->line('admin_common_edit') != '') echo stripslashes($this->lang->line('admin_common_edit')); else echo 'Edit'; ?>"></a></span>
							<?php }?>
							<?php if ($allPrev == '1' || in_array('3', $cms)){?>	
								<span><a class="action-icons c-delete" href="javascript:confirm_delete('<?php echo ADMIN_ENC_URL;?>/cms/delete_help/<?php echo $row->id;?>')" title="<?php if ($this->lang->line('admin_common_delete') != '') echo stripslashes($this->lang->line('admin_common_delete')); else echo 'Delete'; ?>"><?php if ($this->lang->line('admin_common_delete') != '') echo stripslashes($this->lang->line('admin_common_delete')); else echo 'Delete'; ?></a></span>
							<?php }?>
							</td>
						</tr>
						<?php 
