		...
		<div class="grid_12">
			<div class="widget_wrap">
				<div class="widget_top">
					...
					echo  '<input name="english" type="checkbox" value="en"  checked disabled readonly><a href="' . base_url() .ADMIN_ENC_URL. '/templates/share_pool_invoice_template" style="color:white">English</a>';
					$lang = array();
					foreach ($langList as $row) {
						...
						if ($row->lang_code != 'en') {
							if (isset($translated_languages) && in_array($row->lang_code, $translated_languages)) {
								echo '<input name="' . $row->name . '" type="checkbox" checked value="' . $row->lang_code . '" disabled readonly><span ' . $styling . '>' . $row->name . "</span>" . '<a href="' . base_url() .ADMIN_ENC_URL. '/templates/share_pool_invoice_template/' . $row->lang_code . '" style="color:red" class="lang_edit_sec">' . $EditText . '</a>';
							} else {
								echo '<input name="' . $row->name . '" type="checkbox" value="' . $row->lang_code . '" class=""><span ' . $styling . '>' . $row->name . "</span>" . '<a href="' . base_url() .ADMIN_ENC_URL. '/templates/share_pool_invoice_template/' . $row->lang_code . '" style="color:red" class="lang_edit_sec">' . $EditText . '</a>';
							}
						}
					}
					?>
				...
				<div class="widget_content">
					<?php 
						...
						echo form_open(ADMIN_ENC_URL.'/templates/insertEditSharePoolInvoicetemplate',$attributes) 
					?>
